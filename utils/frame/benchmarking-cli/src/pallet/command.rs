		...
		ExecDispatch: NativeExecutionDispatch + 'static,
	{
		...
		for benchmark in list.iter_mut() {
			if pallet_names.iter().filter(|p| *p == &benchmark.pallet).count() > 1 {
				...
				benchmark.pallet_display_name = Some(format!("{}_{}",
				String::from_utf8(benchmark.pallet.clone()).expect("pallet name is utf8"),
				instance_name).into_bytes());
			} else {
				benchmark.pallet_display_name = Some(benchmark.pallet.clone());
			}
		}
		...
		list.iter()
			.filter(|item| pallet.is_empty() || pallet == &b"*"[..] || pallet == &item.pallet_display_name.as_ref().unwrap()[..])
			.for_each(|item| {
