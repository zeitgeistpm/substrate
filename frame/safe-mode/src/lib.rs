...
pub mod pallet {
	...
	pub trait Config: frame_system::Config {
		...
		type EnableOrigin: EnsureOrigin<Self::Origin, Success = (BalanceOf<Self>, Self::BlockNumber)>;
		type ExtendOrigin: EnsureOrigin<Self::Origin, Success = (BalanceOf<Self>, Self::BlockNumber)>;
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn enable(origin: OriginFor<T>) -> DispatchResult {
			let (who, _, _) = T::EnableOrigin::ensure_origin(origin)?;
		...
		pub fn extend(origin: OriginFor<T>) -> DispatchResult {
			let (who, _, _) = T::ExtendOrigin::ensure_signed(origin)?;
