...
impl<B, Client> NetworkBehaviourEventProcess<CustomMessageOutcome> for Behaviour<B, Client>
where
	B: BlockT,
	Client: HeaderBackend<B> + 'static,
{
	fn inject_event(&mut self, event: CustomMessageOutcome) {
		match event {
			...
			CustomMessageOutcome::PeerConnected {
				...
				received_handshake,
				negotiated_fallback,
			} => self.events.push_back(BehaviourOut::PeerConnected {
				peer_id,
				received_handshake,
				negotiated_fallback,
			}),
	...
	Client: HeaderBackend<B> + 'static,
{
	...
	) -> Poll<NetworkBehaviourAction<BehaviourOut, <Self as NetworkBehaviour>::ConnectionHandler>> {
